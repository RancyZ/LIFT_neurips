...
```bash
git clone https://github.com/MeeraPatel2703/LIFT.git
cd LIFT
...
Open `http://localhost:8000`, upload your dataset (CSV or XLSX), fill in the column names, and paste your OpenRouter API key. Results stream in real time and are saved to `outputs/`.
...
|-----|---------|-------------|
| `llm.model` | `qwen/qwen3-235b-a22b` | Any model slug from [openrouter.ai/models](https://openrouter.ai/models) |
| `pipeline.test_split` | `0.5` | Train/test split ratio |
| `pipeline.cv_folds` | `5` | Cross-validation folds for hyperparameter tuning |
| `evaluation.shap_top_H` | `5` | Number of top features used in explainability score |
